...
class MessagesController extends Controller
{
     ...
     */
     ...
     public function index($id = null)
     {
         ...
         // إذا تم توفير قيمة $id، ابحث عن المستخدم باستخدام الـ ID الممرر
         if ($id !== null) {
             $users = User::find($id);
     
             if ($users) {
                 // تم العثور على المستخدم باستخدام الـ ID الممرر
                 return view('Chatify::pages.app', [
                     'id' => $id,
                     'messengerColor' => $messenger_color ? $messenger_color : Chatify::getFallbackColor(),
                     'dark_mode' => Auth::user()->dark_mode < 1 ? 'light' : 'dark',
                     'users' => $users, // يمرر المستخدم الذي تم العثور عليه إلى الواجهة
                 ]);
             } else {
                 // يمكنك إضافة منطق خاص إذا لم يتم العثور على المستخدم
             }
         }
     
         // إذا لم يتم توفير قيمة $id أو إذا لم يتم العثور على المستخدم
         // يمكنك استخدام القائمة الافتراضية للمستخدمين أو إضافة منطق خاص
         $users = User::all();
         ...
         return view('Chatify::pages.app', [
             ...
             'messengerColor' => $messenger_color ? $messenger_color : Chatify::getFallbackColor(),
             'dark_mode' => Auth::user()->dark_mode < 1 ? 'light' : 'dark',
             'users' => $users, // قائمة المستخدمين الافتراضية
         ]);
     }
