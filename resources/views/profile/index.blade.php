...
@endif  
    ...
    <div class="col-9 pt-5">
        <div>
           ...
           <!-- Change id="follow" to class="follow" -->
           <input type="hidden" value="{{ $user->id }}" id="usert">
<div class="post" data-user-id="{{ $user->id }}">
    <button class="follow bg-{{$color}}-500 hover:bg-{{$color}}-700 text-white font-semibold py-2 px-4 rounded-full shadow-md" data-user-id="{{ $user->id }}">
        {{$text}}
    </button>
</div>
...
<script>
    $(document).ready(function () {
            ...
            var follow = document.getElementById('usert').value;

 
            // Store a reference to the button clicked
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data == 'oui') {
                        // Use the reference to the clicked button
                        $('#follow').css('background','red');
                    $('#follow').text('unfollow');
                    } else {
                        // Use the reference to the clicked button
                        $('#follow').css('background','green');
                    $('#follow').text('follow');
                      }
                },
                error: function () {
        ...
        $(document).on('click', '.follow', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data == 'oui') {
                        // Use the reference to the clicked button
                        clickedButton.css('background', 'red');
                        clickedButton.text('unfollow');
                    } else {
                        // Use the reference to the clicked button
                        clickedButton.css('background', 'green');
   ...
   <div>
    <div class="row">
      @include('profile.follow')

  
   <div class=" md:w-1/2 flex flex-wrap -mx-2 pt-4">
    @foreach($user->post as $post)
    <div class="w-full sm:w-1 md:w-1 lg:w-3/4 xl:w-3/4 px-2 pb-5">
                ...
                @if(auth()->user()->id == $user->id)
                    <div class="absolute top-0 right-0 p-2">
                        ...
                        <div class="hidden menu bg-white border border-gray-300 absolute top-8 right-0 py-2 rounded-lg shadow">
                          <form method="POST" action=" {{ route('post.detele',$post->id)}}">

                          @csrf
  @method('delete')
                            <button  class="block px-4 py-2 text-gray-800 hover:bg-gray-200">Delete</button>
                          </form>  
